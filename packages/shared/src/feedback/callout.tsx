...
const calloutVariant = cva("rounded-2xl py-6 px-8 mb-4 ", {
  variants: {
    variant: {
...
const Callout = ({
  ...
  return (
    <Flex
      settings={{ shouldTakeSameSpace: false, align: "center" }}
      className={cn(calloutVariant({ variant, sizes }), props.className)}
    >
      {variant && variantIcon[variant] ? variantIcon[variant]() : null}
      <span className="inline-flex mt-1">{children}</span>
    </Flex>
  );
};
